<template>
  <div class="un-input__wrapper">
    ...
    <label v-if="label" class="un-input__label" :for="id">
      <span class="u-truncate" v-html="label" />
    </label>

    <slot />

    <ul v-if="errors.collect(inputName)" class="un-input__errors__container">
      <li
        v-for="(error, i) in errors.collect(inputName)"
        :key="i"
...
export default {
  ...
  created() {
    ...
    if (this.required) {
      ...
      set(
        dict,
        `custom[${this.inputName}].required.validationMessage`,
    ...
    if (this.type === "email") {
      ...
      set(
        dict,
        `custom[${this.inputName}].email.validationMessage`,
        this.validationMessage
      );
    }
...
.un-input {
  ...
  // default styling
  width: 100%;
  padding: ($verticalPadding + ($labelSize * 2/3)) $horizontalPadding
...
.un-input__label {
  ...
  transition: font-size 0.1s ease-in-out, transform 0.1s ease-in-out;
  font-size: $fontSize;
