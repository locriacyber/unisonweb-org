<template>
  <un-docs>
    <div class="un-doc-page">
      <un-content ref="content" :content="$page.docPage.content" />
      <un-doc-sidebar ref="sidebar" :headings="$page.docPage.headings" />
    </div>
  </un-docs>
</template>
  ...
  methods: {
    setContentMinHeight() {
      if (window.matchMedia(`(min-width: ${breakpoints.md.min})`).matches) {
        ...
        const height =
          this.$refs["sidebar"].$refs["getSidebarHeight"].offsetHeight;
  ...
  mounted() {
    if (process.isClient) {
      ...
      window.addEventListener(
        "resize",
        debounce(this.setContentMinHeight, 300)
      );
    }
  },
  components: {
